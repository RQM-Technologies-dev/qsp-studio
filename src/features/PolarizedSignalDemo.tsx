...
import { generatePolarizationPath } from '../math/polarization';
...
export function PolarizedSignalDemo({ params, currentTime, tip, showBasis, opacity = 1, couplingStrength = 1, receiverYaw = 0, receiverPitch = 0, showExcitation = false }: PolarizedSignalDemoProps) {
  const { amplitude, ellipticity, polarization } = params;

  // Frame axes fade as coupling decreases — minimum 0.55 so the geometry
  // remains readable even at worst misalignment.
  ...
  return (
    <group rotation={[receiverPitch, receiverYaw + Math.PI / 2, 0]}>
